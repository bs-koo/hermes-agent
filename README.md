...
- **자동 수집** — 백그라운드 수집기가 AWS·Dooray·캘린더를 주기적으로 모아 SQLite에 적재합니다. 화면과 AI는 DB만 읽으므로 **빠르게(3초 내)** 응답합니다.
- **조용한 알림** — 이상 징후(위험/주의)가 **새로 뜰 때만** Google Chat으로 푸시하고, 해소되면 정상화 1회. 평소엔 조용합니다(노이즈·깜빡임 억제).
- **AI 채팅** — Gemini가 수집된 운영 데이터를 근거로 한국어로 답합니다(스트리밍).
...
핵심 원칙: **비용이 큰 외부 호출(AWS·Gemini)은 백그라운드 수집 계층에서만** 일어납니다. API·프론트·채팅은 **SQLite만 읽어** 빠르고, AWS 비용도 줄입니다. 스케줄러가 단일이어야 하므로 **`uvicorn --workers 1`** 이 전제입니다.
